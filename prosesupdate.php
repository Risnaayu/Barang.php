...
nama='$nama',kategori='$kategori',satuan='$satuan',harga='$harga',hrg_jual='$hargajual' WHERE kode='$kode'")or die(mysql_error());
if($update){
//echo 'BERHASIL';
...
$update=mysqli_query($koneksi,"UPDATE tb_barang SET
nama='$nama',kategori='$kategori',harga='$harga',satuan='$satuan',hrg_jual='$hargajual',photo='$namafile' WHERE kode='$kode'")or die(mysql_error());
if($update){
echo "<script>alert('DATA BERHASIL DI
UPDATE');window.location='index.php';</script>";
}else{
...
UPDATE');window.location='index.php';</script>";
    ...
    PERBOLEHKAN');window.location='editbrg.php?id=$npm';</script>";
    }
    }
    }
    ?>